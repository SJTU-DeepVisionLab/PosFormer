...
import torch
from comer_pos_version1.datamodule.dataset import CROHMEDataset
...
import sys
public_path = '/home/bml/storage/mnt/v-5b11ab9b2a534d9b/org/linchengyu/back-tam/'
...
def data_iterator(
    ...
    i = 0
    for fname, fea, lab in data:                   #fname:图名 fea：图  lab：公式label   这里是在做数据预处理
...
class Batch:
    ...
    def to(self, device) -> "Batch":        #把数据拷到device(gpu)上
        return Batch(
...
class CROHMEDatamodule(pl.LightningDataModule):
    def __init__(
        ...
        zipfile_path: str = f"{public_path}data_crohme.zip",
        test_year: str = "2014",
    ...
    def train_dataloader(self):
        return DataLoader(
            self.train_dataset,
            shuffle=True,             #是否打乱
            num_workers=self.num_workers,    #多线程读数据
            collate_fn=collate_fn,     #合并
        )

...
import numpy as np
from comer_pos_version1.datamodule import vocab, vocab_size , label_make
from comer_pos_version1.model.pos_enc import WordPosEnc
from comer_pos_version1.model.transformer.arm import AttentionRefinementModule
from comer_pos_version1.model.transformer.transformer_decoder import (
    TransformerDecoder,
    TransformerDecoderLayer,
)
from comer_pos_version1.utils.generation_utils import DecodeModel, PosDecodeModel
...
class PosDecoder(PosDecodeModel):
    def __init__(
        ...
        # self.pos_embed = nn.Embedding(7, d_model, padding_idx=0) #[2b,l*5]  -->  [2b,l*5,256]
        # self.segment_embed = nn.Embedding(200+1, d_model, padding_idx=0)  # [2b,l*5]  -->  [2b,l*5,256]
        ...
        # self.pos_proj = nn.Linear(d_model, 7)
    ...
    # def _bulid_pos_label(self, pos_tgt, need_fliter_index, padding_value, sep_value=None, sos_value=None, eos_value=None):
    #     '''

    #     Args:
    #         pos_tgt: (b, l, 5)
    #         need_fliter_index: (b, l, 5)

    #     Returns:
    #         pad_pos_tgt: (b, l, 5)
    #     '''
    #     new_labels = []
    #     for i in range(len(need_fliter_index)):
    #         if sos_value == None:
    #             new_label = [torch.as_tensor(1)]
    #         else:
    #             new_label = [torch.as_tensor(sos_value)]
    #         for index, value in enumerate(need_fliter_index[i]):
    #             sub_label = pos_tgt[i][index][value]
    #             if len(sub_label) >= 1:
    #                 new_label.extend(pos_tgt[i][index][value])
    #                 if sep_value == None:
    #                     new_label.append(new_label[-1])
    #                 else:
    #                     new_label.append(torch.as_tensor(sep_value))
    #         new_label = torch.stack(new_label, dim=0)
    #         if eos_value != None:
    #             new_label[-1] = torch.as_tensor(eos_value)
    #         new_labels.append(new_label)
    #     pad_pos_tgt = torch._C._nn.pad_sequence(new_labels, batch_first=True, padding_value=padding_value)
    #     return pad_pos_tgt
